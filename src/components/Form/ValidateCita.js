...
export default function Validate(values) {
  ...
  if (!values.area_code) {
    ...
    errors.area_code = "Ingrese el código de área sin 0 al inicio";
  ...
  if (!values.phone) {
    ...
    errors.phone = "Ingrese el número sin 15 al inicio";
  ...
  //cita_date
  const cita = document.getElementById("cita_date");

  if (!cita.value.length) {
    errors.cita_date = "Seleccione fecha y hora";
  }
  ...
  if (cita.value.length) {
    ...
    const citaDia = new Date(fecha).getDay();

    if (values.donde === "madero" && citaDia === 6) {
      errors.cita_date =
        "Sin atención los Sábados en las Oficinas de Puerto Madero";
    } else if (citaDia === 0) {
      errors.cita_date = "Sin atención los Domingos";
    }
  }
